...
//c represents socket, which automatically connects
var server = net.createServer(function(socket) { //'connection' listener
  ...
  socket.on('data', function(connection) {
    ...
    var path = (connection.toString().split(' ')[1]);
    console.log(path);

    var statusline = "HTTP/1.1 200 OK" + "\n";

    var writeHeader = function(servername) {
      var date = new Date();
      var UTCString = date.toUTCString();
      return "server:" + servername + "\n" + "date:" + UTCString +  "\n"+ "\n";
    };

    var header = writeHeader("serverBondage");
